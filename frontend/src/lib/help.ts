...
const LIMITATIONS: Partial<Record<string, string[]>> = {
  ...
  'unlock-pdf': ['Filozy can remove protection only when the document can be opened with the password or permissions you are authorized to use.'],
...
export function helpQuickAnswer(tool: Tool): string {
  return `${tool.searchName} is a browser-based Filozy tool built for this task: ${tool.blurb} It runs on this device, creates a separate result, and does not require an account or document upload.`;
}
